...
url = "https://store-site-backend-static.ak.epicgames.com/freeGamesPromotions?locale=ru&country=US"
...
try:
    ...
    output_lines.append(f"Всего элементов в ответе (US): {len(elements)}\n")
        ...
        if has_promotions:
            ...
            if upcoming_offers:
                for group in upcoming_offers:
                    for offer in group.get("promotionalOffers", []):
                        ds = offer.get("discountSetting", {})
                        output_lines.append(f"    - Скидка (будущая): тип={ds.get('discountType')}, значение={ds.get('discountValue')}, процент={ds.get('discountPercentage')}, даты={offer.get('startDate')} -> {offer.get('endDate')}")
        output_lines.append("-" * 50)
        
    with open("egs_inspect_us.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(output_lines))
    print("Результат успешно записан в egs_inspect_us.txt")
